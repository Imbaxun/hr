...
import './KitchenSchedulingManage.css'
import moment from 'moment';
const { MonthPicker} = DatePicker;
const { IP, ClassManageUrl} = API

class KitchenSchedulingManage extends Component{
  constructor(props){
    super(props)
    this.state = {
      schedulingName : '',
  ...
  startData = () =>{
    let url = `${IP}${ClassManageUrl}?page=0&size=10&schedulingType=kitchen`
    getfun(url).then(res =>{
      console.log(res)
      let arr = res.content
      arr.forEach(item =>{
  ...
  changePage = (page, pageSize) =>{
    ...
    let url =`${IP}${ClassManageUrl}?page=${page-1}&size=${pageSize}&month=${month}&year=${year}&schedulingName=${schedulingName}&schedulingType=kitchen`
    getfun(url).then(res => {
  ...
  searchData = () =>{
    ...
    let url = `${IP}${ClassManageUrl}?schedulingName=${schedulingName}&month=${month}&year=${year}&page=0&size=10&schedulingType=kitchen`
    /*
    ...
    return (
      <div>
        ...
        <div className="comMain">
          <h3 className="comtitle">卤味厨房班次维护列表</h3>
...
export default KitchenSchedulingManage;