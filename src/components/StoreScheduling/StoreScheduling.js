...
class StoreScheduling extends Component{
  constructor(props) {
    super(props)
    this.state={
      name: '',
  ...
  changePage = (page, pageSize) =>{
    ...
    let aa = searchUrl === '' ? `${IP}${ClassSearchUrl}?page=${page-1}&size=${pageSize}&month=${searchmonth}&year=${searchyear}&empName=${name}&storeCode=${code}&schedulingName=${schedulingName}&recordType=store` : `${searchUrl}&page=${page-1}&size=${pageSize}&month=${searchmonth}&year=${searchyear}&empName=${name}&storeCode=${code}&schedulingName=${schedulingName}&recordType=store`
    let url = aa
    getfun(url).then(res => {
  ...
  getThreeData = (item) =>{
    ...
    //点击查询的url
    let searchUrl = `${IP}${ClassSearchUrl}?page=0&size=10&${item}&year=${ayear}&month=${amonth}&recordType=store`
  ...
  onChangeDepClassMonth = (date, dateString) =>{
    ...
    let perUrl = `${IP}/employeeScheduling/store/${date._d.getFullYear()}/${date._d.getMonth()+1}`
    // console.log(perUrl)
  ...
  personClass = () =>{
    ...
    const {searchmonth, searchyear} = this.state
    let perUrl = `${IP}/employeeScheduling/store/${searchyear}/${searchmonth}`
    // console.log(perUrl)
    getfun(perUrl).then(res => this.setState({perClass:res})).catch(err =>console.log(err))
        ...
        <Row type="flex" justify="space-around">
          ...
          <hr />
            <div className="comMain">
              <h3 className="comtitle">门店班次查询列表</h3>
