...
class workRule extends Component {
  ...
  submit =() =>{
    const{time1,time2,time3,time4,time5,time6,time7,time8,time9} = this.state
    let sendArr = []
    sendArr[0] = time1
    sendArr[1] = time2
    sendArr[2] = time3
  ...
  render() {
    ...
    return(
      <div>
        ...
        <h3 className="comtitle">签到/签退设置（门店无效）</h3>
        <Row>
          <Col>
        ...
        <div style={{display:'flex',marginTop:10}}>
          ...
          <Input defaultValue={time9} onChange={e => this.inputChange('time9', e.target.value)}   disabled={this.state.read}  style={{width:80,marginLeft:10,marginRight:10}}/>
